...
#################################################################
    ...
    def create_main_head(self):
        ...
        btn_quit = MyButton(self.main_head_frame,self.data_manager, text=self.language_dict['back'], command=self.create_record_page.back, width=8)
        btn_quit.pack(side = "left", padx=10, pady=5)

        if self.modus == 'new_record':
            header_text = self.language_dict['new_record']
        elif self.modus == 'edit_record':
            header_text = self.language_dict['edit_record']
        else:
