...
class AccountFrame:
    ...
    def create_main_frame(self,container):
        ...
        self.lbl_empty0 = MyLabel(self.main_frame, self.data_manager, width=2)
        self.lbl_empty0.pack(side='right',padx=3)

        if self.account_dict['account_kind'] == 1:
            self.btn_action = MyButton(self.main_frame, self.data_manager, width=16)
            if status != 'closed':
            ...
            self.btn_action = MyLabel(self.main_frame, self.data_manager, width=16)
            status_text = 'Unterkonto'

        self.btn_action.pack(side='right',padx = 3,pady=10)

        
        self.lbl_empty1 = MyLabel(self.main_frame, self.data_manager, width=2)
        self.lbl_empty1.pack(side='right',padx=3)

        self.lbl_status = MyLabel(self.main_frame, self.data_manager,text = status_text, width=15)
        self.lbl_status.pack(side='right',padx=3)

        self.lbl_process = MyLabel(self.main_frame, self.data_manager,text = self.account_dict['process_nbr'], width=15)
        self.lbl_process.pack(side='right',padx=3)

        self.lbl_order = MyLabel(self.main_frame, self.data_manager,text = self.account_dict['order_nbr'], width=15)
        self.lbl_order.pack(side='right',padx=3)

        self.lbl_project = MyLabel(self.main_frame, self.data_manager,text = self.account_dict['project_nbr'], width=15)
        self.lbl_project.pack(side='right',padx=3)

        self.lbl_empty2 = MyLabel(self.main_frame, self.data_manager, width=2)
        self.lbl_empty2.pack(side='left',padx=3)
        
        if self.account_dict['account_kind'] == 0:
            self.lbl_name = MyLabel(self.main_frame, self.data_manager, text = '     ' + self.account_dict['name'] + '   (Hauptkonto: ' +  self.account_dict['main_name'] +')', anchor='w')
        else:
            self.lbl_name = MyLabel(self.main_frame, self.data_manager, text = self.account_dict['name'], anchor='w')
        self.lbl_name.pack(side='left',padx=3)

        self.on_account = False

        self.main_frame.bind("<Enter>", self.enter_account)
        self.main_frame.bind("<Leave>", self.leave_account)

        self.main_frame.bind("<Button-1>", self.activate_account)
...
##################################################
    ...
    def activate_account(self,e=None):
        if self.accounts_tab.get_clicked_account_dict() == self.account_dict:
            self.accounts_tab.reset_clicked_account_dict()
        else:
            self.accounts_tab.set_clicked_account_dict(self.account_dict)
        self.account_list.update()
        self.accounts_tab.head.update()
        self.update()
