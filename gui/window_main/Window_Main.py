...
class MainWindow(tk.Toplevel):
    ...
    def create_main_frame(self):
        ...
        def main_window_leave(e):
            if self.highlight_main_window == True:
                self.highlight_main_window = False
                self.main_frame.configure(highlightbackground=self.style_dict["border_color"], highlightcolor=self.style_dict["border_color"], highlightthickness=1)

        self.main_frame.bind("<Leave>", main_window_leave)
        ...
        self.lbl_icon.bind('<Button-1>', self.tips_and_tricks)
        ...
        self.lbl_icon.bind("<Enter>", enter_icon)
        self.lbl_icon.bind("<Leave>", leave_icon)

        dt = datetime.now()
        str_today = dt.strftime("%d") + "." + dt.strftime("%m") + "." + dt.strftime("%Y") + "  KW" + str(dt.isocalendar()[1])
        self.lbl_title = MyLabelPixel(self.title_bar, self.data_manager, text='   EasyTARC                     ' + str_today)
    ...
    def tips_and_tricks(self,event):
        text = """
Tipps & Tricks:

Das Programm setzt sich aus 5 Tabs (Erfassung, Buchen, Erfasste Zeiten, Zeitkonten und Einstellung) und zwei Arbeitsfenster (mini_work_window und bar_work_window) zusammen.
...
---------------------------
Hintergrundiformationen zu gespeichtern Daten:

---------------------------
Hintergrundiformationen zur Lizensierung:
        """
